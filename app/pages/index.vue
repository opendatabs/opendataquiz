...
const deptOptions = [
    'Präsidialdepartement','Bau- und Verkehrsdepartement','Erziehungsdepartement','Finanzdepartement',
    'Gesundheitsdepartement','Justiz- und Sicherheitsdepartement','Departement für Wirtschaft, Soziales und Umwelt',
    'Sonstiges im Kanton (Parlamentsdienste etc.)','Extern'
] as const
...
const questions = ref<Q[]>([
    {
        ...
        answer:1,
        explanation:'Mit 35.7 °C wurde bisher am 2. Juli die höchste Lufttemperatur im Jahr 2025 gemessen. Am 13. August war die Temperatur mit 35.4 °C tiefer, aber mit einer Luftfeuchtigkeit von 61 %... ',
    ...
    },
    {
        ...
        answer:1,
        explanation:'Die Wohnviertel sind: Altstadt Grossbasel, Vorstädte, Am Ring, Breite, St. Alban, Gundeldingen, Bruderholz, Bachletten, Gotthelf, Iselin, St. Johann, Altstadt Kleinbasel, Clara, Wettstein, Hirzbrunnen, Rosental, Matthäus, Klybeck, Kleinhüningen, Riehen, Bettingen.',
    ...
    },
    {
        ...
        answer:0,
        explanation:'Am 3. August 2025 wurde der höchste Wasserstand im Rhein mit 247.379 m gemessen.',
    ...
    },
    {
        ...
        answer:0,
        explanation:'In den letzten Grossratswahlen 2024 wurden 46% der Sitze von Frauen gewonnen, was den höchsten Anteil in der Geschichte der Wahlen darstellt.',
    ...
    },
    {
        ...
        answer:2,
        explanation:'Die Rangliste ist : 1) Luna, 2) Nala, 3) Rocky, 4) Kira, 5) Mia',
    ...
    },
    {
        ...
        answer:0,
        explanation:'Die Stadtreinigung reinigt jährlich mehr als 1000 Sprayereien an öffentlichen Anlagen und privaten Gebäuden. Im Jahr 2021 wurden 808 Sprayereien an öffentlichen Anlagen entfernt, was den höchsten Wert in den letzten Jahren darstellt.',
        dataset: 'Sprayereien'
    },
    {
        ...
        answer:1,
        explanation:'Es gibt mehere Strassennamen, die in zwei der drei Gemeinden vorkommen darunter der Ausserbergweg, die Grenzacherstrasse und der Krumme Weg. Aber es gibt keinen Strassennamen, der in allen drei Gemeinden vorkommt.',
    ...
    },
    {
        ...
        answer:2,
        explanation: '',
        dataset:'Eheschliessungen nach Trauungsdatum; Kantonaler Stromverbrauch; Kennzahlen zu den Basler Wohnvierteln und Landgemeinden; Ordnungsbussen'
    },
    {
        ...
        answer:1,
        explanation:'316 Datensätze sind im kantonalen Datenportal des Kantons Basel-Stadt aktuell verfügbar. Diese Zahl ändert sich laufend, da neue Datensätze hinzugefügt und bestehende aktualisiert werden.',
...
const isCorrect = computed(() => submitted.value && selected.value === questions.value[idx.value].answer)

// Fortschritt: nur abgesendete Fragen zählen
const progressValue = computed(() => Math.min(idx.value + (submitted.value ? 1 : 0), questions.value.length))
const progressMax = computed(() => questions.value.length)

const runId = ref<string>(`${Date.now()}-${Math.random().toString(36).slice(2,8)}`)
...
function finalMessage(score:number){
    if (score === 0) return 'Uff – noch Luft nach oben. Versuch’s gleich nochmal! 💪'
    if (score <= 3) return 'Solider Start! Weiter so – beim nächsten Mal wird’s mehr. 📈'
